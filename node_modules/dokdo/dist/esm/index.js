...
// src/commands/js.ts
import { Embed, EmbedBuilder, Collection as Collection3, Attachment, ButtonBuilder as ButtonBuilder3, ButtonStyle as ButtonStyle2 } from "discord.js";
...
// src/commands/jsi.ts
import { Collection as Collection4, ButtonBuilder as ButtonBuilder5, ButtonStyle as ButtonStyle4 } from "discord.js";
...
async function curl(message2, parent2) {
  ...
  let res2;
  try {
    const response = await request(message2.data.args.split(" ")[0]);
    const text = await response.body.text();
    try {
      type = "json";
      res2 = JSON.stringify(JSON.parse(text), null, 2);
    } catch {
      type = HLJS.getLang(response.headers["content-type"]) || "html";
      res2 = text;
    }
  } catch (e) {
    type = "js";
...
async function cat(message2, parent2) {
  ...
  fs.readFile(filename, async (err, data) => {
    ...
    await msg2.addAction([
      ...
      {
        button: new ButtonBuilder7().setStyle(ButtonStyle6.Secondary).setCustomId("dokdo$stop").setLabel("Stop"),
        action: ({ manager }) => manager.destroy(),
        requirePage: true
      },
      {
  ...
  async run(ctx) {
    if (ctx instanceof Message7) {
      ...
      if (!ctx.data.args && ctx.attachments.size > 0 && !this.options.disableAttachmentExecution) {
        ...
        if (!file)
          return;
        const text = await request2(file.url).then((res2) => res2.body.text());
        const type = { ext: file.name.split(".").pop(), fileName: file.name };
        if (type.ext && ["txt", "js", "ts", "sh", "bash", "zsh", "ps"].includes(type.ext)) {
          ctx.data.args = text;
...
__name(Dokdo, "Dokdo");
var src_default = { Client: Dokdo, Utils: utils_exports, Commands: commands_exports };
export {
  Dokdo as Client,
